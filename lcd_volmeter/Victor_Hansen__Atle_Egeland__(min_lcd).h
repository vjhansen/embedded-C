...
Dette er et bibliotek for LCD-displayet EA DOGM163 (4-bit/5V) 16x2.
...
void LCD_HALF(unsigned char RS, unsigned char RW, unsigned char data){
	...
	// - Toggling Enable-bit to send data.
	enable();                                                       // - Aktiverer enable funksjon.
}
...
void lcd_string(unsigned char row, unsigned char column, char *string) {
	set_cursor(row, column);							// - Her setter vi hvilken rad og kolonne teksten skal vises p�...
	while(*string) {								// - ..deretter looper vi gjennom strengen...
												
		LCD_FULL(1,0,*string++);                                                // - ..og skriver dette til LCD-displayet med LCD_FULL.
		_delay_us(28);
	}
}
...
void LCD_clear () {
	LCD_FULL(0,0,CLEAR_DISP); _delay_ms(2);			// - Sending instruksjonen 'CLEAR DISPLAY' til LCD_FULL.
}
