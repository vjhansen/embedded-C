/*******************************************************************************
Project 		TWI Slave Rx demonstration (send switches value to TWI Slave)
Target 			ATmega1281 on STK300
Program			TWI_SLAVE_Rx_1281_C.c
...
int main( void )
{
	...
	USART_DisplayString_With_NewLine_StartAndEnd("TWI Slave Rx Demo");
	USART_DisplayString_With_NewLine_StartAndEnd("******************");
...
ISR(TWI_vect)	// TWI Interrupt Handler (Wake up Slave)
{
	...
	unsigned char TWI_DR; // Store TWI data register value
	char cReceivedDataString[100];
	...
	switch(TWI_SR)
	{
		case TWI_S_SLA_W_ACK:	// 0x80 Data has been recd and Ack'd (Slave unique address recognised)
			USART_DisplayString_With_NewLine_AtEnd("Slave address recognised");
			break;

		case TWI_S_GenCall_DATA_ACK:	// 0x90 Data has been recd and Ack'd (General Call recognised)
			USART_DisplayString_With_NewLine_AtEnd("General call recognised");
			break;

		case TWI_S_SLA_W_DATA_ACK:		// The W signifies the Master has Written data to us
		case TWI_S_SLA_W_DATA_NACK:
			TWI_DR = TWDR;
			sprintf(cReceivedDataString,"Master sent %3d",TWI_DR);
			USART_DisplayString_With_NewLine_AtEnd(cReceivedDataString);
			PORTB = ~TWI_DR;
			break;
	}
	TWINT_Clear();
