/*******************************************************************************
Project 		TWI Master Tx demonstration (send switches value to TWI Slave)
Target 			ATmega1281 on STK300
Program			TWI_MASTER_Tx_1281_C.c
...
Function		Reads switches and displays value on LEDs
				Transmits switches values to TWI slave at rate of 4Hz
				Transmits switches values to PC Hyperterminal (as diagnostic), using the microcontroller's Serial port (USART0)
				
				LEDs connected to Port B
				Switches connected to Port C
*******************************************************************************/
...
void TWI_Master_Send_Switches_Value();
...
int main( void )
{
	...
	USART_DisplayString_With_NewLine_StartAndEnd("TWI Master Tx Demo");
	USART_DisplayString_With_NewLine_StartAndEnd("******************");
	InitialiseTimer3();
...
// Verify behaviour with Hyperterminal connected to serial port
	unsigned char uSwitchesValue;
	uSwitchesValue = ~PINC;
	TWI_SendSTART();
	TWI_Send_SLA_W(TWI_SLAVE_address);	// Write to Slave	
	TWI_MasterSendDataByte(uSwitchesValue);
	TWI_Send_STOP();
	char cSwitchesString[100];
	sprintf(cSwitchesString,"    Switches: %3d",uSwitchesValue);
	USART_DisplayString_With_NewLine_AtEnd(cSwitchesString);
	PORTB = ~uSwitchesValue;
}
...
{
	TWI_Master_Send_Switches_Value();
}
//**************************************************************
